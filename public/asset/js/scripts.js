...
$(function () {
    ...
    /** Sidebar submenu visibility: inline !important beats theme CSS (e.g. li.active > ul { display:block }). */
    var setSidebarSubmenuDisplay = function ($menus, visible) {
        $menus.each(function () {
    ...
    var sidebarBindDropdownClicks = function () {
        if ($(".main-sidebar").length) {
            ...
            $(".main-sidebar .sidebar-menu li a.has-dropdown")
                ...
                .on("click", function (e) {
                    ...
                    var submenu = parentLi.children("ul.dropdown-menu, .dropdown-menu");
                    var isOpen = parentLi.hasClass("open");

                    e.preventDefault();

                    if (isOpen) {
                        parentLi.removeClass("open manual-collapsed");
                        me.removeClass("toggled");
                        setSidebarSubmenuDisplay(submenu, false);
                    } else {
                        parentLi.siblings(".dropdown").each(function () {
                            var $sib = $(this);
                            $sib.removeClass("open manual-collapsed");
                            $sib.children("a.has-dropdown").removeClass("toggled");
                            setSidebarSubmenuDisplay(
                                $sib.children("ul.dropdown-menu, .dropdown-menu"),
                                false
                            );
                        });
                        parentLi.addClass("open").removeClass("manual-collapsed");
                        me.addClass("toggled");
                        setSidebarSubmenuDisplay(submenu, true);
                    }
    ...
    /** Route-active top sections only — open panel with inline display so reopen/close stays reliable. */
    var sidebarSyncRouteActivePanels = function () {
        $(".main-sidebar .sidebar-menu > li.dropdown.active").each(function () {
            var $li = $(this);
            $li.addClass("open");
            $li.children("a.has-dropdown").first().addClass("toggled");
            setSidebarSubmenuDisplay(
                $li.children("ul.dropdown-menu, .dropdown-menu"),
                true
            );
        });
    };

    /** Ensure EMR Master Data item is active when on its route. */
    var sidebarSyncEmrMasterData = function () {
        var currentPath = (window.location.pathname || "").toLowerCase();
        if (currentPath.indexOf("/emrmasterdata") === -1) {
            return;
        }

        $(".emrMasterDataSideA, .emrMasterDataStandaloneSideA").addClass("active");
    };

    sidebarSyncRouteActivePanels();
    sidebarSyncEmrMasterData();
    sidebarBindDropdownClicks();
