...
class DoctorController extends Controller
{
    ...
    function fetchDoctorPlansAndSlots(Request $request)
    {
        ...
        $doctor = Doctors::with(['bankAccount', 'category'])->find($request->doctor_id);
        ...
        $mondaySlots = array_values(array_filter($slots->toArray(), function ($slot) {
            return $slot['weekday'] == 1;
        }));
        $tuesdaySlots = array_values(array_filter($slots->toArray(), function ($slot) {
            return $slot['weekday'] == 2;
        }));
        $wednesdaySlots = array_values(array_filter($slots->toArray(), function ($slot) {
            return $slot['weekday'] == 3;
        }));
        $thursdaySlots = array_values(array_filter($slots->toArray(), function ($slot) {
            return $slot['weekday'] == 4;
        }));
        $fridaySlots = array_values(array_filter($slots->toArray(), function ($slot) {
            return $slot['weekday'] == 5;
        }));
        $saturdaySlots = array_values(array_filter($slots->toArray(), function ($slot) {
            return $slot['weekday'] == 6;
        }));
        $sundaySlots = array_values(array_filter($slots->toArray(), function ($slot) {
            return $slot['weekday'] == 7;
        }));

        $sortDaySlots = function (&$daySlots) use ($allowedSlotPosition, $normalizeSlotTime) {
            $daySlots = array_values(array_filter($daySlots, function ($slot) use ($allowedSlotPosition, $normalizeSlotTime) {
                $normalized = $normalizeSlotTime($slot['time']);
                return $normalized !== null && isset($allowedSlotPosition[$normalized]);
            }));

            usort($daySlots, function ($a, $b) use ($allowedSlotPosition, $normalizeSlotTime) {
                return $allowedSlotPosition[$normalizeSlotTime($a['time'])] <=> $allowedSlotPosition[$normalizeSlotTime($b['time'])];
            });
        };

        $sortDaySlots($mondaySlots);
        $sortDaySlots($tuesdaySlots);
        $sortDaySlots($wednesdaySlots);
        $sortDaySlots($thursdaySlots);
        $sortDaySlots($fridaySlots);
        $sortDaySlots($saturdaySlots);
        $sortDaySlots($sundaySlots);
        ...
        $doctorPlans = DoctorPlans::where('is_deleted', 0)
                        ...
                        ->map(function ($plan) use ($request){
                            ...
                            $doctorIds = json_decode($plan->doctor_ids, true) ?? [];
                            $plan->doctors_under_plan =  Doctors::with(['bankAccount', 'category'])
    ...
    function date_wise_slot(Request $request)
    {
        ...
        $doctor_slots = DoctorAppointmentSlots::where('doctor_id', $request->doctor_id)
            ...
            ->get();
        $slots = [];
        foreach ($doctor_slots as $key => $slot) {
            $slot->is_booked = 0;
            $isBooked = Appointments::where('doctor_id', $request->doctor_id)
    ...
    function fetchDoctorProfile(Request $request)
    {
        ...
        $doctor_with_same_category = Doctors::where('category_id', $doctor->category_id)->where('id','!=',$doctor->id)->get();
    ...
    function doctorRegistration(Request $request)
    {
        ...
        if ($doctor == null) {
            ...
            // $doctor->device_type = $request->device_type;
            // $doctor->save();
            return GlobalFunction::sendSimpleResponse(false, 'Doctor with same identity already exist');
        }
    }
    ...
    function doctorLogin(Request $request)
    {
        ...
        {
            if($doctor->status == 1){
                $doctor = Doctors::where('status',1)->where('identity', $request->identity)->where('password',$request->password)->first();
                if($doctor)
                {
