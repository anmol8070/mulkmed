...
class AppointmentController extends Controller
{
    ...
    function fetchMyAppointments(Request $request)
    {
        ...
        $result = Appointments::with(['user', 'patient', 'doctor', 'documents', 'prescription', 'rating', 'appointmentMeeting'])
        ...
        foreach ($result as $appointment) {
            ...
            $appointment->jitsi_link = $jitsiMeeting?->link;
    ...
    function fetchAppointmentDetails(Request $request)
    {
        ...
        $result->jitsi_link = $jitsiMeeting?->link;
        $result->ai_vital_link = null;
        $ai_vital = AI_Vital::where('user_id',$result->user_id)->Where('appointment_id', $result->id)->orderBy('id','desc')->first();
        if($ai_vital)
        {
    ...
    function fetchAcceptedAppointsByDate(Request $request)
    {
        ...
        foreach ($result as $appointment) {
            ...
            $parts = parse_url($jitsiMeeting?->doctor_link);
            $serverURL = $parts['scheme'] . '://' . $parts['host'];
            parse_str($parts['query'] ?? '', $queryParams);

            $jitsi_link =[
                'serverURL' => $serverURL,
                'room'      => $queryParams['roomId'] ?? null,
                'token'     => $queryParams['jwt'] ?? null,
            ];

            $appointment->jitsi_link = $jitsi_link;
