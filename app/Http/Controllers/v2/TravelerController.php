...
class TravelerController extends Controller
{
    public function getServiceTypes(Request $request)
    {
        ...
        $Banner = TravelFlowBanner::select('id','tourist_partner_banner as image')->where('banner_type','travel_guide')->get();
        $buildLocalizedData = function () {
            return [
        ...
        $data = $buildLocalizedData();
        $currentLocale = app()->getLocale();
        $localized_data = [];

        foreach (['en', 'hi', 'ur', 'ar', 'fr'] as $locale) {
            app()->setLocale($locale);
            $localized_data[$locale] = $buildLocalizedData();
        }
    ...
    public function initiateTravelerPayment_old(Request $request)
    {
        // try {
            ...
            if($request->service_type == 2)
            {
                ...
                // Nights = checkout - checkin (no +1). Clamp to minimum 1 night.
                $numberOfDays                       = $checkIn->diffInDays($checkOut, false);
    ...
    public function initiateTravelerPayment(Request $request)
    {
        ...
        foreach ($request->tourist_list as $touristIndex => $touristData) {
            if ((int) ($touristData['service_type'] ?? 0) === 2) {
                $dateValidator = Validator::make($touristData, [
                    'check_in_time' => 'required|date|after_or_equal:today',
                    'check_out_time' => 'required|date|after:check_in_time',
                ], [
                    'check_in_time.after_or_equal' => "The selected check-in date is not valid. Please choose today’s date or a future date.",
                    'check_out_time.after' => "The selected check-out date is not valid. Please ensure it is later than the check-in date.",
                    'check_in_time.required' => "The selected check-in date is not valid. Please choose today’s date or a future date.",
                    'check_in_time.date' => "The selected check-in date is not valid. Please choose today’s date or a future date.",
                    'check_out_time.required' => "The selected check-out date is not valid. Please ensure it is later than the check-in date.",
                    'check_out_time.date' => "The selected check-out date is not valid. Please ensure it is later than the check-in date.",
                ]);

                if ($dateValidator->fails()) {
                    return response()->json([
                        'status' => false,
                        'message' => $dateValidator->errors()->first(),
                    ]);
                }
            }

            if ((int) ($touristData['service_type'] ?? 0) === 1) {
                $dateValidator = Validator::make($touristData, [
                    'fly_in' => 'required|date|after_or_equal:today',
                    'fly_out' => 'required|date|after:fly_in',
                ], [
                    'fly_in.after_or_equal' => "The selected fly-in date is not valid. Please choose today’s date or a future date.",
                    'fly_out.after' => "The selected fly-out date is not valid. Please ensure it is later than the fly-in date.",
                    'fly_in.required' => "The selected fly-in date is not valid. Please choose today’s date or a future date.",
                    'fly_in.date' => "The selected fly-in date is not valid. Please choose today’s date or a future date.",
                    'fly_out.required' => "The selected fly-out date is not valid. Please ensure it is later than the fly-in date.",
                    'fly_out.date' => "The selected fly-out date is not valid. Please ensure it is later than the fly-in date.",
                ]);
            ...
            if ($touristData['service_type'] == 2) {
                ...
                // Charge based on nights (no +1). Example: 2026-03-25 -> 2026-03-26 = 1 night.
                $numberOfDays = $checkIn->diffInDays($checkOut, false);
                $numberOfDays = ($numberOfDays > 0) ? $numberOfDays : 1;
            ...
            // Hotel flow: consultation count should be per-night.
            // Other flows keep existing day/validity based count.
            if ((int) $touristData['service_type'] === 2) {
                $tourist->number_of_consultation = $numberOfDays;
            } else {
                $tourist->number_of_consultation = $numberOfDays;
            }

            // ✅ Per Night Amount (service_type=2 uses nights)
            // $amountPerDay = 1;
