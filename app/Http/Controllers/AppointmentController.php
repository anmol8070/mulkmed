...
class AppointmentController extends Controller
{
    ...
    function scheduleAppointmentReminders(Request $request){
        ...
        ->with(['user', 'patient', 'doctor', 'admindocuments', 'rating', 'prescription'])
        ->first();
        if ($appointment == null) {
    ...
    function viewAppointment($id)
    {
        $item = Appointments::where('id', $id)
            ->with(['user', 'patient', 'doctor', 'admindocuments', 'prescription', 'rating'])
            ->first();

        $item->documents = $item->admindocuments;
        ...
        // $orderSummary['taxes'] = json_decode($orderSummary['taxes'], true) ?? [];
        $orderSummary['taxes'] = is_array($orderSummary['taxes']) ? $orderSummary['taxes'] : (json_decode($orderSummary['taxes'], true) ?? []);
        $prescription = null;
    ...
    function fetchMissedAppointmentsList(Request $request)
    {
        $totalData =  Appointments::where('status', Constants::orderMissed)->count();
        $rows = Appointments::where('status', Constants::orderMissed)->orderBy('id', 'DESC')->get();
        $settings = GlobalSettings::first();
        ...
        $totalFiltered = $totalData;
        if (empty($request->input('search.value'))) {
            $result = Appointments::where('status', Constants::orderMissed)->offset($start)
                ->limit($limit)
                ->orderBy($order, $dir)
                ->get();
        } else {
            $search = $request->input('search.value');
            $result =  Appointments::where('status', Constants::orderMissed)->where(function ($query) use ($search) {
                $query->Where('appointment_number', 'LIKE', "%{$search}%")
                    ->orWhere('payable_amount', 'LIKE', "%{$search}%");
            })->offset($start)
                ->limit($limit)
                ->orderBy($order, $dir)
                ->get();
            $totalFiltered = Appointments::where('status', Constants::orderMissed)->where(function ($query) use ($search) {
                $query->Where('appointment_number', 'LIKE', "%{$search}%")
                    ->orWhere('payable_amount', 'LIKE', "%{$search}%");
            })->count();
        }
    ...
    function fetchMyAppointments(Request $request)
    {
        ...
        $result = Appointments::with(['user', 'patient', 'doctor', 'admindocuments', 'prescription', 'rating'])
        ...
        foreach ($result as $appointment) {
            $appointment->previous_appointments =
                Appointments::with(['user', 'patient', 'doctor', 'admindocuments', 'prescription', 'rating'])
                ->Where('user_id', $request->user_id)
    ...
    function declineAppointment(Request $request)
    {
        ...
        $appointment = Appointments::where('id', $request->appointment_id)
            ->with(['user', 'patient', 'doctor', 'admindocuments'])
            ->first();
        if ($appointment == null) {
            return GlobalFunction::sendSimpleResponse(false, 'Appointment does not exists!');
        }
    ...
    function fetchAppointmentDetails(Request $request)
    {
        ...
        $result = Appointments::where('id', $request->appointment_id)
            ->with(['user', 'patient', 'doctor', 'admindocuments', 'prescription', 'rating', 'rating'])
            ->first();
        if ($result == null) {
            return GlobalFunction::sendSimpleResponse(false, 'Appointment does not exists!');
        }
    ...
    function addAppointment(Request $request)
    {
        ...
        $appointment = Appointments::where('id', $appointment->id)->with(['user', 'doctor', 'patient', 'admindocuments'])->first();
    ...
    function changeExpertise(){
        //  Initialize translators just once
    // $trAr = new GoogleTranslate('ar');
    // $trFr = new GoogleTranslate('fr');
    // $trHi = new GoogleTranslate('hi');
    // $trUr = new GoogleTranslate('ur');

    // $expertises = DoctorExpertise::all();

    // foreach ($expertises as $item) {
    //     // Only translate if not already translated
    //     if (empty($item->ar_title)) {
    //         $item->ar_title = $trAr->translate($item->title);
    //     }
    //     if (empty($item->fr_title)) {
    //         $item->fr_title = $trFr->translate($item->title);
    //     }
    //     if (empty($item->hi_title)) {
    //         $item->hi_title = $trHi->translate($item->title);
    //     }
    //     if (empty($item->ur_title)) {
    //         $item->ur_title = $trUr->translate($item->title);
    //     }
