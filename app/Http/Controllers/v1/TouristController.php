...
class TouristController extends Controller
{
    ...
    public function touristLogin(Request $request)
    {
        ...
        if(isset($tourist))
        {
            ...
            // {
            //     $checkIn  = Carbon::parse($tourist->fly_in);
            //     $checkOut = Carbon::parse($tourist->fly_out);
            //     $now      = Carbon::now();
    ...
    $checkIn  = Carbon::parse($tourist->fly_in)->startOfDay();
    $checkOut = Carbon::parse($tourist->fly_out)->endOfDay();
    $now      = Carbon::now();

    // ✅ Check current booking window
    if ($now->between($checkIn, $checkOut)) {

        $tourist->update([
            'status' => 1,
            'expiry_date' => $tourist->fly_out
        ]);
    ...
    $touristFallback->update([
        'status' => 1,
        'expiry_date' => $touristFallback->fly_out
    ]);

    return response()->json([
        'status' => true,
        'data' => $touristFallback
    ]);
}
            ...
            elseif($tourist->agent_type == 2)
            {
                ...
                // if ($now->between($checkIn, $checkOut))
                // Allow login on:
                // - the previous day of `check_in_time`
                // - the full check-in and check-out days (DB stores timestamps, but auth is day-based)
                $windowStart = $checkIn->copy()->subDay()->startOfDay();
                $windowEnd   = $checkOut->copy()->endOfDay();
    ...
    public function updateTouristFCM(Request $request)
    {
        try {
            ...
            $cleanToken = trim((string) $validated['device_token']);
            if ($cleanToken === '') {
                return response()->json([
                    'status'  => false,
    ...
    public function homePage(Request $request)
    {
        ...
        $sectionSequence[] = [
            'section_name' => 'Mulk Symptom Checker',
...
Team Mulk Med";            
    ...
    {        
        ...
        if (is_array($pregnancies)) {
            foreach ($pregnancies as &$pregnancy) {
                $name = is_array($pregnancy)
                    ? ($pregnancy['pregnancy_name'] ?? '')
                    : ($pregnancy->pregnancy_name ?? '');

                $normalized = trim((string) $name);
                if (strcasecmp($normalized, '(- not specified -)') === 0) {
                    $normalized = "Don't know";
                } elseif ($normalized !== '') {
                    $normalized = ucfirst(strtolower($normalized));
                }

                if (is_array($pregnancy)) {
                    $pregnancy['pregnancy_name'] = $normalized;
                } else {
                    $pregnancy->pregnancy_name = $normalized;
                }
            }
    ...
    function ranked_differential_diagnoses(Request $request)
    {
        ...
        if ($ranked_differential_diagnoses->ok()) {
            ...
            $this->applyTouristRankedDiagnosesQueryResultDetails(
        ...
        return response()->json([
            'status' => false,
            'message' => 'Failed to fetch ranked differential diagnoses from Isabel',
    ...
    function markMissedAppointmentFromSheduler(Request $request)
    {
        ...
        if ($appointment->status == Constants::orderAccepted) {
            ...
            $appointment->diagnosed_with = $request->diagnosed_with;
            $appointment->save();    
            \Log::info('V1 tourist appointment marked missed', [
                'appointment_id' => $appointment->id,
                'doctor_id' => $appointment->doctor_id,
            ]);

            $appointment->loadMissing(['tourist', 'doctor']);
            $jitsiMeeting = TouristJitsiMeeting::where('appointment_id', $appointment->id)->latest()->first();
            $touristName = trim(
            ...
            $touristCountryCode = GlobalFunction::normalizeCountryCode($appointment->tourist->country_code ?? null);
            $touristPhone = preg_replace('/\D+/', '', (string) ($appointment->tourist->contact_number ?? $appointment->tourist->phone_number ?? ''));
            $touristLabel = $touristName;
            if (!empty($touristPhone)) {
                $touristLabel = trim($touristName . ' +' . ($touristCountryCode ?: '') . $touristPhone);
            }
            ...
            if ($jitsiMeeting && (int) $jitsiMeeting->doctor_joined === 0 && (int) $jitsiMeeting->user_joined === 1) {
                $message = "Dr missed call from {$touristLabel}.";
            } elseif ($jitsiMeeting && (int) $jitsiMeeting->user_joined === 0 && (int) $jitsiMeeting->doctor_joined === 1) {
                $message = "{$touristLabel} missed call from {$doctorName}.";
            } elseif ($jitsiMeeting && (int) $jitsiMeeting->user_joined === 0 && (int) $jitsiMeeting->doctor_joined === 0) {
                $message = "Missed call between {$doctorName} and {$touristLabel}.";
            }

            if ($message) {
                \Log::info('V1 tourist missed SMS prepared', [
                    'appointment_id' => $appointment->id,
                    'message' => $message,
                ]);
                foreach (['971522463433', '971569337544'] as $mobile) {
                    try {
                        \Log::info('V1 tourist missed SMS sending', [
                            'appointment_id' => $appointment->id,
                            'mobile' => $mobile,
                            'message_preview' => $message,
                        ]);
                        EmailHelpers::sendSms($mobile, $message);
                        \Log::info('V1 tourist missed SMS sent', [
                            'appointment_id' => $appointment->id,
                            'mobile' => $mobile,
                            'message_preview' => $message,
                        ]);
                    } catch (\Throwable $e) {
                        \Log::warning('Missed alert SMS failed (tourist v1)', [
                            'mobile' => $mobile,
                            'appointment_id' => $appointment->id,
                ...
                \Log::info('V1 tourist missed SMS skipped due to join-state mismatch', [
                    'appointment_id' => $appointment->id,
    ...
    private function resolveRegionIdForCountryId($countryId): ?string
    {
        ...
        foreach ($countries as $c) {
            if (!is_array($c) || !isset($c['country_id'])) {
                continue;
            }
            if ((string) $c['country_id'] !== $needle) {
                continue;
            }
    ...
    private function applyTouristRankedDiagnosesQueryResultDetails(
