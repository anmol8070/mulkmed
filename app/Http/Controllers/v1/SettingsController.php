...
class SettingsController extends Controller
{
    ...
    {
        if(isset($request->country) && $request->country == "IN")
        {
            // return response()->json(['status' => true, 'url' => 'https://india.mulkmed.com/v2/']);
            return response()->json(['status' => true, 'url' => 'https://indiamulkmed.reapmind.com/']);
        }else{
            // return response()->json(['status' => true, 'url' => 'https://pt.mulkmed.com/v2/']);
             return response()->json(['status' => true, 'url' => 'https://uaemulkmed.reapmind.com/']);
        }
    }
    function sendScheduledReminders_Cron(){
    ...
    function privacy_policy(Request $request){
        // $data = Pages::first();
        $PolicyView = route('appPrivacyView');
        return GlobalFunction::sendDataResponse(true, 'Data fetch successfully', $PolicyView);
    }

    function terms_conditions(Request $request){
        // $data = Pages::first();
        $termsView = route('appviewTerms');
        return GlobalFunction::sendDataResponse(true, 'Data fetch successfully', $termsView);
    }

    function help_center(Request $request){
        // $data = Pages::first();
        $termsView = route('appHelpCenterView');
        return GlobalFunction::sendDataResponse(true, 'Data fetch successfully', $termsView);
    }
}
