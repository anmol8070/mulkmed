...
class UsersController extends Controller
{
    ...
    public function sendUsernameReminder(Request $request)
    {
        ...
        Mail::to($user->identity)->send(new SendUsernameMail($user->username));

        return response()->json(["message" => 'An email with your username has been sent.']);
    }
    ...
    {
        return view('partner_network.index');
    } 

    function fetchPartnerNetwork(Request $request)
    {
        $totalData =  PartnersTable::where('is_deleted', 0)->count();
        $rows = PartnersTable::where('is_deleted', 0)->orderBy('id', 'DESC')->get();
        ...
        $totalFiltered = $totalData;
        if (empty($request->input('search.value'))) {
            $result = PartnersTable::where('is_deleted', 0)->offset($start)
                ->limit($limit)
                ->orderBy($order, $dir)
                ->get();
        } else {
            $search = $request->input('search.value');
            $result =  PartnersTable::where('is_deleted', 0)
                ->Where('title', 'LIKE', "%{$search}%")
                ->offset($start)
                ->limit($limit)
                ->orderBy($order, $dir)
                ->get();
            $totalFiltered = PartnersTable::where('is_deleted', 0)
                ->Where('title', 'LIKE', "%{$search}%")
                ->count();
        }
        $data = array();
        foreach ($result as $item) {
           ...
           $edit = '<a href="#" 
            ...
            data-headline="' . htmlspecialchars($item->headline) . '" 
            data-hospital_name="' . htmlspecialchars($item->hospital_name) . '" 
            data-address="' . htmlspecialchars($item->address) . '" 
            data-website_link="' . htmlspecialchars($item->website_link) . '" 
        ...
        function addPartnerNetwork(Request $request)
    {
        $item = new PartnersTable();
        $item->title = $request->title;
        $item->headline = $request->headline;
        $item->hospital_name = $request->hospital_name;
        $item->address = $request->address;
        $item->website_link = $request->website_link;
       ...
       function editPartnerNetwork(Request $request)
    {   
        $item = PartnersTable::find($request->id);
        $item->title = $request->title;
        $item->headline = $request->headline;
        $item->hospital_name = $request->hospital_name;
        $item->address = $request->address;
        ...
        function deletePartnerNetwork($id)
    {
        $health_problem = PartnersTable::find($id);
        $health_problem->is_deleted = 1;
        $health_problem->save();

        return GlobalFunction::sendSimpleResponse(true, 'Partner Network deleted successfully');
    }

    function ViewPartner($id){
        $partner = PartnersTable::find($id);
        $imgUrl = GlobalFunction::createMediaUrl($partner->image);
        return view('partner_network.viewPartner', compact('partner', 'imgUrl'));
    }

    function viewVitalReport(){
        $filename = "pages.vitalScanReport.pdf";
    //  return view('pages.vitalScanReport');
    $pdf = PDF::loadView('pages.vitalScanReport', [
        // pass real data here if needed: 'patient' => $patient, ...
    ])
